...
func TestSearchBookByISBN(t *testing.T) {
    ...
    if err != nil {
        t.Fatalf("HTTP call failed, did you check the connection? got this: %v", err)
        return
    }
    ...
    if err != nil {
	t.Errorf("Couldn't read response.body, got this error: %v", err)
	return
    }
...
func TestSearchByAuthor(t *testing.T) {
    ...
    if err != nil {
        t.Fatalf("HTTP Request failed, did you check the connection? Got: %v", err)
    }
    client := &http.Client {}
    res, err := client.Do(req)
    if err != nil {
        t.Fatalf("HTTP call failed, did you check the connection? Got: %v", err)
        return
    }
    if res.StatusCode >= 200 && res.StatusCode <= 299 {
...
func TestCreateBookWithInvalidParameters(t *testing.T) {
  ...
  if string(body) != `{"success": "created"}` {
    ...
    req, err := http.NewRequest(method, url+isbn, nil)
    if err != nil {
        t.Errorf("HTTP call failed, got: %v", err)
        return
    }
    res, err := client.Do(req)
    if err != nil {
    ...
    res.Body.Close()
    t.Logf("The irregularly created book has been deleted by its ISBN")
  }
}
